...
//	@GetMapping("/{donor_id}")
//	public ResponseEntity<Donor> findById(@PathVariable Long donorId ){
...
//		}
	...
	public ResponseEntity<List<Donor>> findByAddressAndBloodGroup(@PathVariable("address")String address, @PathVariable("bloodGroup")String bloodGroup){
		...
		if(donor1.isEmpty()) {
			return ResponseEntity.notFound().build();//<String>("donor not found ",HttpStatus.BAD_REQUEST);
			
	//	return new ResponseEntity<List>(donor1,HttpStatus.OK);
		}else {
			return ResponseEntity.ok(donor1); // <List>(donor1,HttpStatus.OK);
			//return new ResponseEntity<String>("donor not found ",HttpStatus.BAD_REQUEST);
		}
	}
